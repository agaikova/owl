...
class Owl(ctn_benchmark.Benchmark):
    def params(self):
        ...
        self.default('time glasses are on', t_glasses_on = 100)
    ...
    def model(self, p):
        ...
        with model:
            ...
            visual_raw = nengo.Ensemble(n_neurons=300, dimensions=3, intercepts = nengo.dists.Uniform(p.left_intercept,p.right_intercept))
            auditory = nengo.Ensemble(n_neurons=200, dimensions = 2, intercepts = nengo.dists.Uniform(p.left_intercept,p.right_intercept))
            ...
            visual_cor = nengo.Ensemble(n_neurons = 200, dimensions = 2, intercepts = nengo.dists.Uniform(p.left_intercept,p.right_intercept))
    ...
    def evaluate(self, p, sim, plt):
        ...
        def degree_to_itd(degree_val):
            return int(degree_val * 2.5)
        ...
        #pylab.plot(shift)
        width_a = [(np.where(data1>0))[0][0], (np.where(data1>0))[-1][-1]]
        middle = int(np.mean(width_a))
        pos_dist = (np.where(data1>0))[-1][-1] - middle
        neg_dist = middle - (np.where(data1>0))[0][0]
        curve_width = (np.where(data1>0))[-1][-1] - (np.where(data1>0))[0][0]
        
        return {'shift':shift, 'pos_dist': pos_dist, 'neg_dist':neg_dist, 'curve_width':curve_width}
